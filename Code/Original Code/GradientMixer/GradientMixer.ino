...
void setup() {
  /* Initial setup
   *  This part function only runs ones 
   */
  Serial.begin(9600);
  ...
  /* To control the RF outlet, you need to get the
   *  pulse length, protocole, and binary code.
   *  You can get this with the example file "ReceiveDemo_Advanced" from the library rc-switch
   *  and connecting a receiving antena to the arduino.
   */
  ...
  // Attach an interrupt to the ISR vector (interrupt buttons)
  attachInterrupt(digitalPinToInterrupt(StopButton), PauseProgram,  RISING);

  //Display the main menu and declaring we are at position 0 which is the main menu
  Mainmenu();
  currentPage = '0'; 
}
...
void loop(){
  ...
  //  y=myTouch.getY();
    ...
    if (currentPage == '0'){ // Main Page
      if (myTouch.dataAvailable()){ //if touch detected
       // get x and y values
        myTouch.read();
        x=myTouch.getX();
        y=myTouch.getY();
        if ((y >= 95) && (y <= 135) && (x >= 35) && (x <= 285)){
          waitForIt(35, 95, 285, 135);//animation of selected button
    ...
    if (currentPage == '1'){ // Start Page
      if (myTouch.dataAvailable()){
        ...
        if((x >= 5) && (x <= 70)){//Checks in which state button is selected and redraw the buttons with the selected option on red
          if((y >= 55) && (y <= 85)){
      ...
      if (myTouch.dataAvailable()){
        ...
        if ((y >= 170) && (y <= 210)){  // OFF & ON position in x
          if ((x>=22) && (x<=153)){  // Button:  OFF
            ...
            myGLCD.drawRoundRect (100,70, 220, 130);// Draws rounded rectangle without a fill, so the overall appearance of the button looks like it has a frame
            pump = 0;
            mySwitch.send("10000001000010000101111000000000"); //Change the binary code to your 433Hz wireless outlet binary code
            mySwitch.send("10000001000010000101111000000000"); //Change the binary code to your 433Hz wireless outlet binary code
          ...
          if ((x>=167) && (x<=298)){  // Button: ON
            ...
            myGLCD.drawRoundRect (100,70, 220, 130);// Draws rounded rectangle without a fill, so the overall appearance of the button looks like it has a frame
            pump = 1;
            mySwitch.send("10001110000010000101111000000000"); //Change the binary code to your 433Hz wireless outlet binary code
            mySwitch.send("10001110000010000101111000000000"); //Change the binary code to your 433Hz wireless outlet binary code
    ...
    if (currentPage == '5'){ // Process
      ...
      if (once == 0){
        ...
        //Calculates the times for the iterations between 0 and t1
        timeA = percentageA * 60;
        timeB = 6000 - timeA;

        //Prints in which cycle is at
        ...
        //Indicates on the screen that Valve is on
        valve = 1;
        myGLCD.setColor(255, 255, 0);// Sets Yellow color
        ...
        //Turns Valve on and mantains it on for timeA ms
        itoa ((timeA - 15),buffer,10);
        myGLCD.print(buffer,(75), 93);
        digitalWrite(VALVE, HIGH);
        delay(timeA - 15);// the -15 comes for compensating the delay of the valve  
        
        //Indicates on the screen that Valve is off
        valve = 0;
        myGLCD.setColor(255, 255, 0);
        myGLCD.fillRoundRect (175,70, 285, 130);
        myGLCD.setColor(0, 0, 0);// Sets Black color
        myGLCD.fillRoundRect (35,70, 145, 130);// Draws filled rounded rectangle
        //Turns Valve off and mantains it off for timeB ms
        itoa ((timeB + 5),buffer,10);
        myGLCD.print(buffer,(210), 93);
        digitalWrite(VALVE, LOW);
        delay(timeB + 5);// the +5 comes for compensating the delay of the valve

        //Same thing as before, but this time taking different values for valve delay compensation
        ...
        myGLCD.setColor(255, 255, 255); // Sets color to white
        myGLCD.print("DONE!", 35, 150); // Prints the string //Done of stage 1
        ...
        for (int x = 0; x <= n2; x++){
            ...
            //Formulas for second stage (Linear increment)
